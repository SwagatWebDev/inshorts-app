...
const NewsCard = ({ imageUrl, title, details, link, shortDetails }) => {
    ...
    return (
        <div className={`max-w-md bg-white rounded-xl overflow-hidden shadow-md m-2 flex-grow flex flex-col swipe-${swipeDirection}`}
            ...
            {/* Body Section */}
            <div className="flex-grow px-6 py-3 overflow-hidden" style={{ maxHeight: '240px' }}>
                <h2 className="text-md font-bold mb-2">{title}</h2>
                <p className="text-gray-700 text-sm" style={{ height: '240px', overflow: 'hidden', textOverflow: 'ellipsis', display: '-webkit-box', WebkitLineClamp: 6, WebkitBoxOrient: 'vertical' }}>
                    {details}
                </p>
            </div>
            {/* Horizontal line */}
